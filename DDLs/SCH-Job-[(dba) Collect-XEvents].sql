...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'(dba) Collect-XEvents', 
		...
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N'This job consumes data from XEvent [xevent_metrics] into dbo.xevent_metrics.
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'dbo.usp_collect_xevent_metrics', 
		@step_id=1, 
		...
		@command=N'sqlcmd -E -b -S localhost -d DBA -Q "EXEC dbo.usp_collect_xevent_metrics;"', 
		@flags=40
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
